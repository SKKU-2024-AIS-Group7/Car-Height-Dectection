...
import base64
from utils import (
    HeightPredictionModel,
    detect_objects,
    segment_objects,
    draw_boxes_and_masks,
)
...
def model_fn(model_dir):
    ...
    yolov5_model = torch.hub.load(
        "ultralytics/yolov5", "custom", path=hf_path, force_reload=True
    ).to(device)

    mask_rcnn_model = torchvision.models.detection.maskrcnn_resnet50_fpn_v2(
        pretrained=True
    ).to(device)
    ...
    regression_model.load_state_dict(
        torch.load(os.path.join(model_dir, "height_prediction_model.pth"))
    )
...
def predict_fn(input_object, model):
    ...
    masks, outputs = segment_objects(
        mask_rcnn_model, image_rgb, vehicle_boxes, device, scaler, regression_model
    )
    ...
    return {"heights": heights, "widths": widths, "image_bytes": image_bytes}
